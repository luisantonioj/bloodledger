exports.up = (pgm) => {
  pgm.sql(`
    CREATE TABLE app.v2_donations (
      ...
      CONSTRAINT v2_donations_key CHECK (donation_number_key_version ~ '^[A-Z0-9_-]{1,31}$'),
    ...
    CREATE TABLE app.v2_components (
      ...
      CONSTRAINT v2_components_reservation CHECK ((inventory_status = 'RESERVED' AND reservation_purpose IS NOT NULL AND reservation_id IS NOT NULL) OR (inventory_status <> 'RESERVED' AND reservation_purpose IS NULL AND reservation_id IS NULL)),
      CONSTRAINT v2_components_preparation CHECK (release_prepared_at IS NULL OR (release_prepared_by IS NOT NULL AND inventory_status = 'RESERVED')),
    ...
    CREATE TABLE app.v2_reservations (
      ...
      CONSTRAINT v2_reservations_status CHECK (status IN ('RESERVED','PREPARED','DISPATCHED','RELEASED','CANCELLED')),
