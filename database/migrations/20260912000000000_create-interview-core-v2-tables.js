exports.up = (pgm) => {
  pgm.sql(`
    ...
    CREATE TABLE app.v2_components (
      ...
      CONSTRAINT v2_components_reservation CHECK ((inventory_status IN ('RESERVED','DISPATCHED','IN_TRANSIT','RECEIVED','RECONCILIATION_HOLD') AND reservation_purpose IS NOT NULL AND reservation_id IS NOT NULL) OR (inventory_status NOT IN ('RESERVED','DISPATCHED','IN_TRANSIT','RECEIVED','RECONCILIATION_HOLD') AND reservation_purpose IS NULL AND reservation_id IS NULL)),
      CONSTRAINT v2_components_preparation CHECK (release_prepared_at IS NULL OR (release_prepared_by IS NOT NULL AND inventory_status IN ('RESERVED','DISPATCHED','IN_TRANSIT','RECONCILIATION_HOLD'))),
