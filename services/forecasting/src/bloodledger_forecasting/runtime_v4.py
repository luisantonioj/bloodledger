...
V4_SERIES = tuple((blood_type, component) for blood_type in V4_BLOOD_TYPES for component in V4_COMPONENTS)
...
def _canonical_rows(data: pd.DataFrame) -> list[dict[str, object]]:
    ...
    for row in data[["business_date", "blood_type", "component", "requested_units"]].itertuples(index=False):
        rows.append({
    ...
    return sorted(rows, key=lambda item: (str(item["business_date"]), str(item["blood_type"]), str(item["component"])))
...
def _unavailable_bundle(
    *,
    ...
    run_identity = _sha256({"dataset": V4_DATASET_VERSION, "input": input_hash, "horizon": horizon, "reason": reason})
    run = {
    ...
    lineage["payloadSha256"] = _sha256({"schemaVersion": V4_BUNDLE_SCHEMA, "run": run, "forecasts": []})
...
def _validate_input(data: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    if missing:
        raise ForecastingError("V4_INPUT_SCHEMA_INVALID", f"Missing V4 input fields: {sorted(missing)}")
    if data.empty:
    ...
    normalized["business_date"] = pd.to_datetime(normalized["business_date"], errors="coerce").dt.date
    if normalized["business_date"].isna().any():
        raise ForecastingError("V4_DATE_INVALID", "V4 business_date contains an invalid value")
    normalized["blood_type"] = normalized["blood_type"].astype(str)
    normalized["component"] = normalized["component"].map(lambda value: V4_COMPONENT_ALIASES.get(str(value), str(value)))
    ...
    if any(not math.isfinite(float(value)) or float(value) < 0 for value in normalized["requested_units"].tolist()):
        raise ForecastingError("V4_QUANTITY_INVALID", "requested_units must be finite and nonnegative")
...
def create_v4_runtime_bundle(
    ...
    *,
    ...
    if not generated_at.endswith("Z"):
        raise ForecastingError("V4_GENERATED_AT_INVALID", "generated_at must be an explicit UTC instant")
    try:
        generated = datetime.fromisoformat(generated_at.replace("Z", "+00:00"))
    except ValueError as error:
        raise ForecastingError("V4_GENERATED_AT_INVALID", "generated_at must be ISO-8601") from error
    ...
    try:
        ...
        if error.code in {"V4_HISTORY_UNAVAILABLE", "V4_CATEGORY_UNSUPPORTED"}:
            return _unavailable_bundle(data=data, generated_at=generated_at, reason=error.code, dataset_sha256=_sha256(str(dataset_path) if dataset_path else "in-memory"))
        raise
    dataset_sha256 = hashlib.sha256(dataset_path.read_bytes()).hexdigest() if dataset_path and dataset_path.is_file() else _sha256(V4_DATASET_VERSION)
    dates = sorted(normalized["business_date"].unique())
    if len(dates) < 7:
        return _unavailable_bundle(data=normalized, generated_at=generated_at, reason="V4_HISTORY_UNAVAILABLE", dataset_sha256=dataset_sha256)
    prior_dates = dates[-7:]
    if any(prior_dates[index] - prior_dates[index - 1] != timedelta(days=1) for index in range(1, len(prior_dates))):
        return _unavailable_bundle(data=normalized, generated_at=generated_at, reason="V4_HISTORY_UNAVAILABLE", dataset_sha256=dataset_sha256)
    expected = {(value, component) for value in V4_BLOOD_TYPES for component in V4_COMPONENTS}
    actual = {(str(row.blood_type), str(row.component)) for row in normalized.itertuples() if row.business_date in prior_dates}
    ...
    for blood_type, component in V4_SERIES:
        series = normalized[(normalized["blood_type"] == blood_type) & (normalized["component"] == component)].sort_values("business_date")
        values = [float(value) for value in series[series["business_date"].isin(prior_dates)]["requested_units"].tolist()]
        point = max(0.0, sum(weight * value for weight, value in zip(V4_WEIGHTS, values, strict=True)) / V4_WEIGHT_DENOMINATOR)
        identity = _sha256({"bloodType": blood_type, "component": component, "horizon": horizon, "input": input_hash})
        records.append({
    ...
    identity = _sha256({"dataset": V4_DATASET_VERSION, "input": input_hash, "model": V4_MODEL_VERSION, "horizon": horizon})
    run = {
    ...
    lineage["payloadSha256"] = _sha256({"schemaVersion": V4_BUNDLE_SCHEMA, "run": run, "forecasts": records})
