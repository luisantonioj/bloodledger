...
export async function buildApp(
  ...
  if (sessions) {
    ...
    if (applicationWrites) {
      ...
      app.post<{ Params: { transferId: string } }>("/api/v1/transfers/:transferId/dispatch", async (request) => {
        requireSameOrigin(request,webOrigin);
        const {principal}=await restore(request);
        if(!["ROLE-01","ROLE-02"].includes(principal.roleId)||!permits(principal.roleId,"transfers:write"))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","Transfer dispatch is restricted to authorized source-hospital staff.");
        if(!TRANSFER_PATTERN.test(request.params.transferId))throw new ApiFailure(400,"INVALID_TRANSFER_ID","Transfer ID is invalid.");
        if(!USER_PATTERN.test(principal.userId))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","The authenticated user is outside the ledger actor policy.");
        const idempotencyKey=request.headers["idempotency-key"];
        if(typeof idempotencyKey!=="string"||!IDEMPOTENCY_PATTERN.test(idempotencyKey))throw new ApiFailure(400,"INVALID_IDEMPOTENCY_KEY","A valid Idempotency-Key header is required.");
        ...
        if(!body||keys.length!==expected.length||keys.some((key,index)=>key!==expected[index])||!Number.isSafeInteger(body.expectedVersion)||Number(body.expectedVersion)<1||
          ...
          !validUtcInstant(location.capturedAt)||new Date(location.capturedAt).getTime()>new Date(body.eventTime).getTime())throw new ApiFailure(400,"INVALID_TRANSFER_DISPATCH","Transfer dispatch input is invalid.");
        const evidenceId="LOC_"+sha256({transferId:request.params.transferId,idempotencyKey,phase:"DISPATCH"}).slice(0,40).toUpperCase();
        const locationEvidence=captureSyntheticLocationEvidence({institutionId:principal.institutionId,phase:"DISPATCH",latitude:Number(location.latitude),longitude:Number(location.longitude),accuracyMetres:Number(location.accuracyMetres),source:location.source as "DEVICE"|"FACILITY_FALLBACK",fallbackReason:location.fallbackReason as "DEVICE_UNAVAILABLE"|"PERMISSION_DENIED"|"SIGNAL_UNAVAILABLE"|null,capturedAt:location.capturedAt},evidenceId);
        const payloadSha256=sha256({transferId:request.params.transferId,sourceInstitutionId:principal.institutionId,actorUserId:principal.userId,expectedVersion:body.expectedVersion,eventTime:body.eventTime,correlationId:body.correlationId,locationEvidence});
        const transferEventId="TEVT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"DISPATCH"}).slice(0,40).toUpperCase(),auditEventId="AUDT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"TRANSFER_DISPATCHED"}).slice(0,40).toUpperCase();
        const result=await applicationWrites.dispatchTransfer({transferId:request.params.transferId,sourceInstitutionId:principal.institutionId,actorUserId:principal.userId,expectedVersion:Number(body.expectedVersion),eventTime:body.eventTime,correlationId:body.correlationId,idempotencyKey,payloadSha256,transferEventId,auditEventId,locationEvidence});
        if(!result)throw new ApiFailure(404,"TRANSFER_NOT_FOUND","A transfer was not found in the authorized institution scope.");
        return result;
      });

      app.post<{ Params: { transferId: string } }>("/api/v1/transfers/:transferId/delay", async(request)=>{
        requireSameOrigin(request,webOrigin);
        const{principal}=await restore(request);
        if(!["ROLE-01","ROLE-02","ROLE-03"].includes(principal.roleId)||!permits(principal.roleId,"transfers:write"))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","Transfer delay reporting is restricted to authorized custody participants.");
        if(!TRANSFER_PATTERN.test(request.params.transferId))throw new ApiFailure(400,"INVALID_TRANSFER_ID","Transfer ID is invalid.");
        if(!USER_PATTERN.test(principal.userId))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","The authenticated user is outside the ledger actor policy.");
        const idempotencyKey=request.headers["idempotency-key"];
        if(typeof idempotencyKey!=="string"||!IDEMPOTENCY_PATTERN.test(idempotencyKey))throw new ApiFailure(400,"INVALID_IDEMPOTENCY_KEY","A valid Idempotency-Key header is required.");
        ...
        if(!body||keys.length!==expected.length||keys.some((key,index)=>key!==expected[index])||!Number.isSafeInteger(body.expectedVersion)||Number(body.expectedVersion)<1||body.reasonCode!=="ROUTE_DELAY"||!validUtcInstant(body.eventTime)||new Date(body.eventTime).getTime()>clock().getTime()+60_000||typeof body.correlationId!=="string"||!CORRELATION_PATTERN.test(body.correlationId))throw new ApiFailure(400,"INVALID_TRANSFER_DELAY","Transfer delay input is invalid.");
        const actorRoleId=principal.roleId as "ROLE-01"|"ROLE-02"|"ROLE-03";
        ...
        const transferEventId="TEVT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"DELAY"}).slice(0,40).toUpperCase(),auditEventId="AUDT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"TRANSFER_DELAYED"}).slice(0,40).toUpperCase();
        const result=await applicationWrites.markTransferDelayed({transferId:request.params.transferId,actorInstitutionId:principal.institutionId,actorRoleId,actorUserId:principal.userId,expectedVersion:Number(body.expectedVersion),reasonCode:"ROUTE_DELAY",eventTime:body.eventTime,correlationId:body.correlationId,idempotencyKey,payloadSha256,transferEventId,auditEventId});
        if(!result)throw new ApiFailure(404,"TRANSFER_NOT_FOUND","A transfer was not found in the authorized custody scope.");
        return result;
      });

      app.post<{ Params: { transferId: string } }>("/api/v1/transfers/:transferId/resume", async(request)=>{
        requireSameOrigin(request,webOrigin);
        const{principal}=await restore(request);
        if(!["ROLE-01","ROLE-02"].includes(principal.roleId)||!permits(principal.roleId,"transfers:write"))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","Resuming transfer transit is restricted to authorized source-hospital staff.");
        if(!TRANSFER_PATTERN.test(request.params.transferId))throw new ApiFailure(400,"INVALID_TRANSFER_ID","Transfer ID is invalid.");
        if(!USER_PATTERN.test(principal.userId))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","The authenticated user is outside the ledger actor policy.");
        const idempotencyKey=request.headers["idempotency-key"];
        if(typeof idempotencyKey!=="string"||!IDEMPOTENCY_PATTERN.test(idempotencyKey))throw new ApiFailure(400,"INVALID_IDEMPOTENCY_KEY","A valid Idempotency-Key header is required.");
        const body=request.body as Record<string,unknown>|null,keys=body&&typeof body==="object"?Object.keys(body).sort():[],expected=["correlationId","eventTime","expectedVersion"];
        if(!body||keys.length!==expected.length||keys.some((key,index)=>key!==expected[index])||!Number.isSafeInteger(body.expectedVersion)||Number(body.expectedVersion)<1||!validUtcInstant(body.eventTime)||new Date(body.eventTime).getTime()>clock().getTime()+60_000||typeof body.correlationId!=="string"||!CORRELATION_PATTERN.test(body.correlationId))throw new ApiFailure(400,"INVALID_TRANSFER_RESUME","Transfer resume input is invalid.");
        const payloadSha256=sha256({transferId:request.params.transferId,sourceInstitutionId:principal.institutionId,actorUserId:principal.userId,expectedVersion:body.expectedVersion,eventTime:body.eventTime,correlationId:body.correlationId});
        const transferEventId="TEVT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"RESUME"}).slice(0,40).toUpperCase(),auditEventId="AUDT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"TRANSFER_TRANSIT_RESUMED"}).slice(0,40).toUpperCase();
        const result=await applicationWrites.resumeTransfer({transferId:request.params.transferId,sourceInstitutionId:principal.institutionId,actorUserId:principal.userId,expectedVersion:Number(body.expectedVersion),eventTime:body.eventTime,correlationId:body.correlationId,idempotencyKey,payloadSha256,transferEventId,auditEventId});
        if(!result)throw new ApiFailure(404,"TRANSFER_NOT_FOUND","A transfer was not found in the authorized institution scope.");
        return result;
      });
      ...
      app.post<{ Params: { transferId: string } }>("/api/v1/transfers/:transferId/transit-start", async(request)=>{
        requireSameOrigin(request,webOrigin);
        const{principal}=await restore(request);
        if(!["ROLE-01","ROLE-02"].includes(principal.roleId)||!permits(principal.roleId,"transfers:write"))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","Starting transfer transit is restricted to authorized source-hospital staff.");
        if(!TRANSFER_PATTERN.test(request.params.transferId))throw new ApiFailure(400,"INVALID_TRANSFER_ID","Transfer ID is invalid.");
        if(!USER_PATTERN.test(principal.userId))throw new ApiFailure(403,"AUTH_SCOPE_FORBIDDEN","The authenticated user is outside the ledger actor policy.");
        const idempotencyKey=request.headers["idempotency-key"];
        if(typeof idempotencyKey!=="string"||!IDEMPOTENCY_PATTERN.test(idempotencyKey))throw new ApiFailure(400,"INVALID_IDEMPOTENCY_KEY","A valid Idempotency-Key header is required.");
        const body=request.body as Record<string,unknown>|null,keys=body&&typeof body==="object"?Object.keys(body).sort():[],expected=["correlationId","eventTime","expectedVersion"];
        if(!body||keys.length!==expected.length||keys.some((key,index)=>key!==expected[index])||!Number.isSafeInteger(body.expectedVersion)||Number(body.expectedVersion)<1||!validUtcInstant(body.eventTime)||new Date(body.eventTime).getTime()>clock().getTime()+60_000||typeof body.correlationId!=="string"||!CORRELATION_PATTERN.test(body.correlationId))throw new ApiFailure(400,"INVALID_TRANSFER_TRANSIT","Transfer transit input is invalid.");
        const payloadSha256=sha256({transferId:request.params.transferId,sourceInstitutionId:principal.institutionId,actorUserId:principal.userId,expectedVersion:body.expectedVersion,eventTime:body.eventTime,correlationId:body.correlationId});
        const transferEventId="TEVT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"START_TRANSIT"}).slice(0,40).toUpperCase(),auditEventId="AUDT_"+sha256({transferId:request.params.transferId,idempotencyKey,operation:"TRANSFER_TRANSIT_STARTED"}).slice(0,40).toUpperCase();
        const result=await applicationWrites.startTransferTransit({transferId:request.params.transferId,sourceInstitutionId:principal.institutionId,actorUserId:principal.userId,expectedVersion:Number(body.expectedVersion),eventTime:body.eventTime,correlationId:body.correlationId,idempotencyKey,payloadSha256,transferEventId,auditEventId});
        if(!result)throw new ApiFailure(404,"TRANSFER_NOT_FOUND","A transfer was not found in the authorized institution scope.");
        return result;
      });
