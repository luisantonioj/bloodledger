...
export interface V2CommandStore {
  ...
  markLedgerCommitted(commandId: string, transactionId: string, now: Date): Promise<void>;
...
export class InMemoryV2CommandStore implements V2CommandStore {
  ...
  async enqueue(input: V2CommandInput): Promise<{ command: V2Command; replayed: boolean }> {
    ...
    const command: V2Command = { ...input, payloadSha256: digest, status: "QUEUED", attemptCount: 0, nextAttemptAt: input.acceptedAt, ledgerTransactionId: null, safeErrorCode: null, classification: "SIMULATION_ONLY", updatedAt: input.acceptedAt };
  ...
  async claim(_workerId: string, now: Date, leaseMs = 30_000): Promise<V2Command | null> {
    const command = [...this.commands.values()].filter((item) => ["QUEUED", "RETRY_WAIT"].includes(item.status) && new Date(item.nextAttemptAt) <= now).sort((a, b) => a.acceptedAt.localeCompare(b.acceptedAt) || a.commandId.localeCompare(b.commandId))[0];
    if (!command) return null;
    command.status = "SUBMITTING"; command.attemptCount += 1; command.updatedAt = now.toISOString();
    void leaseMs;
    return command;
  }
  async markLedgerCommitted(commandId: string, transactionId: string, now: Date): Promise<void> { const command = this.must(commandId); command.status = "LEDGER_COMMITTED_PROJECTION_PENDING"; command.ledgerTransactionId = transactionId; command.updatedAt = now.toISOString(); }
...
export class PostgresV2CommandStore implements V2CommandStore {
  ...
  async claim(workerId: string, now: Date, leaseMs = 30_000): Promise<V2Command | null> {
    ...
    try {
      await client.query("BEGIN");
      const result = await client.query<Record<string, unknown>>(`SELECT * FROM app.v2_commands WHERE status IN ('QUEUED','RETRY_WAIT') AND next_attempt_at <= $1 ORDER BY accepted_at,command_id FOR UPDATE SKIP LOCKED LIMIT 1`, [now.toISOString()]);
  ...
  async markLedgerCommitted(commandId: string, transactionId: string, now: Date): Promise<void> { await this.pool.query("UPDATE app.v2_commands SET status='LEDGER_COMMITTED_PROJECTION_PENDING',ledger_transaction_id=$2,ledger_committed_at=$3,lease_owner=NULL,lease_expires_at=NULL,updated_at=$3,version=version+1 WHERE command_id=$1", [commandId, transactionId, now.toISOString()]); }
