...
async function fixture(){const password=randomBytes(24).toString("base64url"),sessions=new MemorySessions(),saltHex=randomBytes(16).toString("hex");sessions.record={userId:"USR_SYNTH_WEB_01",username:"synth_operator_01",displayName:"Synthetic Operator 01",institutionId:"INST_SYNTH_01",institutionDisplayName:"Synthetic Hospital 01",institutionCategory:"HOSPITAL",roleId:"ROLE-01",saltHex,verifierHex:await deriveVerifier(password,saltHex)};const app=await buildApp(new MemoryRepository(),config,()=>new Date(),sessions);return{app,password}}
