...
import { contractVersionFor, readReconciliationReasons, readReservation, readReservations, readV2Components, recoverAcceptedCommand, submitReservationAction, submitV2Reconciliation, type ReconciliationReasons, type V2Component, type V2Reservation } from "../../services/api/v2";

type Action = "prepare" | "dispatch" | "transit" | "receive" | "cancel" | "local-release-complete";
...
export function permittedReservationActions(reservation: V2Reservation, role: Principal["roleId"]): Action[] {
  ...
  if (reservation.status === "DISPATCHED") return source ? ["transit"] : [];
  if (reservation.status === "IN_TRANSIT") return recipient ? ["receive"] : [];
  return [];
}
...
export function ReservationWorkspace({ principal, onRefresh }: { principal: Principal; onRefresh: () => void }) {
  ...
  const actions = selected ? permittedReservationActions(selected, principal.roleId) : [];
  return <section className="v2-operation-card">
    ...
    {selected && <div className="v2-operation-form"><p><strong>{selected.reservationId}</strong> · {humanizeCode(selected.status)} · version {selected.version} · updated {formatManilaDateTime(selected.updatedAt)}</p><p>Prepared evidence: {selected.preparedEvidencePresent ? "recorded" : "not recorded"}. No Donation No. or patient details are shown.</p>
      {actions.length > 0 && <form onSubmit={(event) => void submit(event)}>
        <label>Action<select value={action ?? ""} disabled={busy || Boolean(status.command)} onChange={(event) => { setAction(event.target.value as Action || undefined); attempted.current = undefined; setError(""); }}><option value="">Choose action</option>{actions.map((item) => <option key={item} value={item}>{humanizeCode(item)}</option>)}</select></label>
        ...
        <button className="button primary" disabled={!action || busy || Boolean(status.command)}>{busy ? "Submitting…" : "Confirm action"}</button>
      </form>}
      {["DISPATCHED", "IN_TRANSIT", "RECEIVED"].includes(selected.status) && <p>Compromise submission is unavailable until an approved incident reason-code vocabulary is supplied.</p>}
