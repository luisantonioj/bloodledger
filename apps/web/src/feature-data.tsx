...
function ApproveTransferAction({transfer,onDone}:{transfer:Transfer;onDone:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    ...
    try{const response=await fetch("/api/v1/transfers/"+encodeURIComponent(transfer.transferId)+"/approval",{method:"POST",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json","Idempotency-Key":current.keys.idempotencyKey},body:JSON.stringify(current.payload)});const body=await response.json().catch(()=>null) as {selectedUnitIds?:string[];error?:{message?:string}}|null;if(!response.ok)throw new Error(body?.error?.message??"Transfer approval failed.");setCommitted(true);attempt.current=undefined;onDone()}catch(reason){setError(reason instanceof Error?reason.message:"Transfer approval failed.")}finally{setBusy(false)}
  }
...
function DelayTransferAction({transfer,onDone}:{transfer:Transfer;onDone:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    ...
    try{const response=await fetch("/api/v1/transfers/"+encodeURIComponent(transfer.transferId)+"/delay",{method:"POST",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json","Idempotency-Key":current.keys.idempotencyKey},body:JSON.stringify(current.payload)});const body=await response.json().catch(()=>null) as {error?:{message?:string}}|null;if(!response.ok)throw new Error(body?.error?.message??"Delay reporting failed.");setCommitted(true);attempt.current=undefined;onDone()}catch(reason){setError(reason instanceof Error?reason.message:"Delay reporting failed.")}finally{setBusy(false)}
  }
...
function ReceiveTransferForm({transfer,institutionId,onDone}:{transfer:Transfer;institutionId:string;onDone:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    ...
    try{const response=await fetch("/api/v1/transfers/"+encodeURIComponent(transfer.transferId)+"/receipt",{method:"POST",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json","Idempotency-Key":current.keys.idempotencyKey},body:JSON.stringify(current.payload)});const body=await response.json().catch(()=>null) as {error?:{message?:string}}|null;if(!response.ok)throw new Error(body?.error?.message??"Transfer receipt failed.");setCommitted(true);attempt.current=undefined;onDone()}catch(reason){setError(reason instanceof Error?reason.message:"Transfer receipt failed.")}finally{setBusy(false)}
  }
...
function StartTransitAction({transfer,onDone}:{transfer:Transfer;onDone:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    event.preventDefault();setBusy(true);setError("");setCommitted(false);
    const current=attempt.current??(()=>{const keys=newMutationKeys(),eventTime=new Date().toISOString();return{keys,payload:{expectedVersion:transfer.ledgerVersion,eventTime,correlationId:keys.correlationId}}})();attempt.current=current;
    try{const response=await fetch("/api/v1/transfers/"+encodeURIComponent(transfer.transferId)+"/transit-start",{method:"POST",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json","Idempotency-Key":current.keys.idempotencyKey},body:JSON.stringify(current.payload)});const body=await response.json().catch(()=>null) as {error?:{message?:string}}|null;if(!response.ok)throw new Error(body?.error?.message??"Starting transit failed.");setCommitted(true);attempt.current=undefined;onDone()}catch(reason){setError(reason instanceof Error?reason.message:"Starting transit failed.")}finally{setBusy(false)}
  }
...
function DispatchTransferForm({transfer,onDone}:{transfer:Transfer;onDone:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    ...
    try{const response=await fetch("/api/v1/transfers/"+encodeURIComponent(transfer.transferId)+"/dispatch",{method:"POST",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json","Idempotency-Key":current.keys.idempotencyKey},body:JSON.stringify(current.payload)});const body=await response.json().catch(()=>null) as {locationEvidence?:{evidenceId:string};error?:{message?:string}}|null;if(!response.ok)throw new Error(body?.error?.message??"Transfer dispatch failed.");setCommitted(true);attempt.current=undefined;onDone()}catch(reason){setError(reason instanceof Error?reason.message:"Transfer dispatch failed.")}finally{setBusy(false)}
  }
...
function CancelTransferForm({transfer,onDone}:{transfer:Transfer;onDone:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    event.preventDefault();setBusy(true);setError("");setCommitted(false);
    const current=attempt.current??(()=>{const keys=newMutationKeys(),eventTime=new Date().toISOString();return{keys,payload:{expectedVersion:transfer.ledgerVersion,reasonCode,eventTime,correlationId:keys.correlationId}}})();attempt.current=current;
    try{const response=await fetch("/api/v1/transfers/"+encodeURIComponent(transfer.transferId)+"/cancellation",{method:"POST",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json","Idempotency-Key":current.keys.idempotencyKey},body:JSON.stringify(current.payload)});const body=await response.json().catch(()=>null) as {releasedUnitIds?:string[];error?:{message?:string}}|null;if(!response.ok)throw new Error(body?.error?.message??"Transfer cancellation failed.");setCommitted(true);attempt.current=undefined;onDone()}catch(reason){setError(reason instanceof Error?reason.message:"Transfer cancellation failed.")}finally{setBusy(false)}
  }
...
function TransferDetailView({data,close,canReject,canCancel,canCancelApproved,canDispatch,canStartTransit,canDelay,canReceive,receiptInstitutionId,onDone}:{data:TransferDetail;close:()=>void;canReject:boolean;canCancel:boolean;canCancelApproved:boolean;canDispatch:boolean;canStartTransit:boolean;canDelay:boolean;canReceive:boolean;receiptInstitutionId:string;onDone:()=>void}) {
...
function TransferExplorer({data,canSubmit,canReject,canCancel,canCancelApproved,canDispatch,canStartTransit,canDelay,canReceive,receiptInstitutionId,onRefresh}:{data:Transfers;canSubmit:boolean;canReject:boolean;canCancel:boolean;canCancelApproved:boolean;canDispatch:boolean;canStartTransit:boolean;canDelay:boolean;canReceive:boolean;receiptInstitutionId:string;onRefresh:()=>void}) {
...
export function FeatureData({path,canAcknowledge=false,canSubmitTransfer=false,canRejectTransfer=false,canCancelTransfer=false,canCancelApprovedTransfer=false,canDispatchTransfer=false,canStartTransit=false,canDelayTransfer=false,canReceiveTransfer=false,canCapture=false,principal}:{path:string;canAcknowledge?:boolean;canSubmitTransfer?:boolean;canRejectTransfer?:boolean;canCancelTransfer?:boolean;canCancelApprovedTransfer?:boolean;canDispatchTransfer?:boolean;canStartTransit?:boolean;canDelayTransfer?:boolean;canReceiveTransfer?:boolean;canCapture?:boolean;principal?:ProfilePrincipal}) {
