...
function ResumeTransferAction({transfer,onDone}:{transfer:Transfer;onDone:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    event.preventDefault();setBusy(true);setError("");setCommitted(false);
    const current=attempt.current??(()=>{const keys=newMutationKeys(),eventTime=new Date().toISOString();return{keys,payload:{expectedVersion:transfer.ledgerVersion,eventTime,correlationId:keys.correlationId}}})();attempt.current=current;
    try{const response=await fetch("/api/v1/transfers/"+encodeURIComponent(transfer.transferId)+"/resume",{method:"POST",credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json","Idempotency-Key":current.keys.idempotencyKey},body:JSON.stringify(current.payload)});const body=await response.json().catch(()=>null) as {error?:{message?:string}}|null;if(!response.ok)throw new Error(body?.error?.message??"Resuming transit failed.");setCommitted(true);attempt.current=undefined;onDone()}catch(reason){setError(reason instanceof Error?reason.message:"Resuming transit failed.")}finally{setBusy(false)}
  }
...
function TransferExplorer({data,canSubmit,canReject,canCancel,canCancelApproved,canDispatch,canStartTransit,canDelay,canResume,canReceive,receiptInstitutionId,onRefresh}:{data:Transfers;canSubmit:boolean;canReject:boolean;canCancel:boolean;canCancelApproved:boolean;canDispatch:boolean;canStartTransit:boolean;canDelay:boolean;canResume:boolean;canReceive:boolean;receiptInstitutionId:string;onRefresh:()=>void}) {
